...
from src.models.utils_CFPnet import ConditionalMINE, MINE, AutoRegressiveLSTM
...
class CRN(BRCausalModel):
    ...
    possible_model_types = {'CFPnet','encoder', 'decoder'}
...
class CFP(CRN):
    ...
    def __init__(self, args: DictConfig,
        ...
        self.input_size = self.dim_treatments + self.dim_static_features + self.dim_outcome
        self._init_specific(args.model.CFPnet)
        self.save_hyperparameters(args)
    def prepare_data(self) -> None:
        # Datasets normalisation etc.
        if self.dataset_collection is not None and not self.dataset_collection.processed_data_encoder:
    ...
    def forward(self, batch, detach_treatment=False):
        batch_size = batch['Adjustment_vitals'].size(0)
        lstm_input_confounder = []
        lstm_input_confounder.append(batch['Adjustment_vitals'])
