...
class AutoregressiveSimulation:
    def __init__(self, gamma, num_simulated_hidden_confounders, num_u, num_covariates):
...
#        self.gamma_a_y = gamma
        ...
        self.confounders_coefficients['treatments'] = self.generate_coefficients(
            self.p, matrix_shape=(self.num_confounders, self.num_treatments), treatment_coefficients=True)
    ...
    def generate_confounders_u_single_timestep(self, p, history):
        confounders_history_u = history['confounders_u']
        past_confounders_coefficients_u = self.confounders_coefficients_u['confounders_u']
        history_length = len(confounders_history_u)
        if (history_length < p):
            p = history_length
        confounders_sum = np.zeros(shape=(self.num_u,))
        for index in range(p):
            confounders_sum += np.matmul(past_confounders_coefficients_u[index],
                                         confounders_history_u[history_length - index - 1])
